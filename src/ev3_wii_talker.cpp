                    ...
                    union cwiid_mesg mesg[], struct timespec *timestamp)
{
	...
	for (i=0; i < mesg_count; i++)
	{
		switch (mesg[i].type)
		{
			...
			{
				ROS_DEBUG("Status Report: battery=%d extension=", mesg[i].status_mesg.battery);
				switch (mesg[i].status_mesg.ext_type)
				{
					...
					case CWIID_EXT_CLASSIC:
						ROS_DEBUG("Classic Controller");
						break;
					case CWIID_EXT_BALANCE:
						ROS_DEBUG("Balance Board");
						break;
					case CWIID_EXT_MOTIONPLUS:
						ROS_DEBUG("MotionPlus");
						break;
					default:
						ROS_DEBUG("Unknown Extension");
						break;
				}
				break;
			}
			case CWIID_MESG_BTN:
			{
				ROS_DEBUG("Button Report: %.4X\n", mesg[i].btn_mesg.buttons);
				std_msgs::Int32 msg;
				msg.data = mesg[i].btn_mesg.buttons;
				number_publisher.publish(msg);
				break;
			}
			case CWIID_MESG_ACC:
				ROS_DEBUG("Acc Report: x=%d, y=%d, z=%d\n",
			...
			case CWIID_MESG_IR:
				...
				for (j = 0; j < CWIID_IR_SRC_COUNT; j++)
				{
					...
					{
						valid_source = 1;
						ROS_DEBUG("(%d,%d) ", mesg[i].ir_mesg.src[j].pos[CWIID_X],
								mesg[i].ir_mesg.src[j].pos[CWIID_Y]);
					}
				}
				if (!valid_source)
				{
					ROS_DEBUG("no sources detected");
				}
				ROS_DEBUG("\n");
				break;
			...
			case CWIID_MESG_BALANCE:
				ROS_DEBUG("Balance Report: right_top=%d right_bottom=%d "
